                ...
                if (orderList.size() > 0) {
                    ...
                    if (orderSplits.size() > 0) {
                        for (OrderSplit os : orderSplits) {
                            ...
                            ws.url(SysParCom.JD_QUERY_URL).setContentType("application/x-www-form-urlencoded").post(sb.toString()).thenApply(wsResponse -> {
                                try {
                                    ...
                                    if (!sign_data.equalsIgnoreCase(_sign)) {
                                        ...
                                        if (params.get("is_success").equals("Y")) {
                                            ...
                                            if (params.containsKey("insp_push_status") && params.get("insp_push_status") != null)
                                                os.setPayInspReturnCode(params.get("insp_push_status"));//国检报送状态，对需要单独报送国检的海关有效
                                            if (params.containsKey("insp_push_status_desc") && params.get("insp_push_status_desc") != null)
                                                os.setPayInspReturnMsg(params.get("insp_push_status_desc"));//国检报送状态信息描述
